...
Both must land **in the same PR as the shell**, not after it. A shell that merges first is a
tier 2 change whose own suites never ran, which is the failure the tiers exist to prevent
...
- **Four root-level files the skeleton adds match no tier pattern at all**, so each falls
  through to the catch-all rather than to tier 2. One edit, four entries:

  | File | Why it misses | Lands with |
  |---|---|---|
  | `package-lock.json` | tier 2 lists `pnpm-lock.yaml`; the package manager is npm | the shell |
  | `eslint.config.mjs` | tier 2 lists no `eslint*` | the shell |
  | `next-env.d.ts` | generated by Next, tracked, matches nothing | the shell |
  | `drizzle.config.*` | tier 3 lists `drizzle/**`, which needs a directory | the tier-3 slice |
